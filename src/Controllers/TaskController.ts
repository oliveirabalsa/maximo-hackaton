interface IBook {
  ...
  const getBooks = ({ response }: { response: any }) => { 
    response.body = books // conectar com o banco
  }
  ...
  const updateBook = async ({ params, request, response }: { params: { isbn: string }; request: any; response: any }) => {
    ...
    if (book) {
      ...
      book = { ...book, ...updateInfos} // banco
      books = [...books.filter(book => book.isbn !== params.isbn), book] // banco
      response.status = 200
      response.body = { message: 'OK' }
    } else {
      response.status = 404
